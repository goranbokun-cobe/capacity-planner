...
export default async function OverviewPage() {
  const [people, projects, seniorities, settings, rawHolidays] = await Promise.all([
    ...
    // Fetch all holidays — small dataset (~100–200 rows), no date filter needed
    prisma.holiday.findMany({
      select: { calendarId: true, date: true },
    }),
  ]);

  const colorBands = settings?.colorBands
    ? JSON.parse(settings.colorBands)
    : DEFAULT_COLOR_BANDS;
  const defaultCalcMode = (settings?.defaultCalcMode ?? "weighted") as "weighted" | "full";

  // Build a flat lookup: "calendarId:weekId" → number of holiday days in that week.
  // Computed once on the server; the client receives a plain Record<string, number>.
  ...
  // Convert Date fields to weekId strings so the client component receives plain JSON
